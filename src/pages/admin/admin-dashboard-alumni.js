...
function AdminDashboardAlumniPage() {
    ...
    alumniData.forEach((alumni) => {
      alumni.educations?.forEach((edu) => {
        if (edu.graduation_year) {
          let year = parseInt(edu.graduation_year);

          // ถ้าเป็นวันที่ เช่น "2022-03-15" → แปลงเป็นปี
          if (isNaN(year)) {
            year = new Date(edu.graduation_year).getFullYear();
          }

          // ถ้าเป็นปี ค.ศ. → บวก 543 เป็นปี พ.ศ.
          if (year < 2500) {
            year += 543;
          }
  ...
  // ฟิลเตอร์ข้อมูลตามปี, สาขา, และชื่อ
  ...
  const handleAlumniClick = (userId) => {
    navigate(`/admin-home/dashboard-alumni/${userId}`);
  };
...
export default AdminDashboardAlumniPage;
